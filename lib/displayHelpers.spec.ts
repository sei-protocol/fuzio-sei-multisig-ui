...
describe('printableCoin', () => {
  ...
  it('works with coin in ChainInfo', () => {
    expect(printableCoin(coinInChain, testChainInfo)).toEqual(
      `0.001${thinSpace}JUNOX`,
    );
  });

  it("works with coin not in ChainInfo with leading 'u'", () => {
    expect(printableCoin(coinNotInChainLeading, testChainInfo)).toEqual(
      `0.02${thinSpace}TEST`,
    );
  });
  ...
  it('works with IBC coin', () => {
    expect(printableCoin(ibcCoin, testChainInfo)).toEqual(
      `4000000${thinSpace}ibc/c4cff…319f9`,
    );
  });
});
...
describe('printableCoins', () => {
  ...
  it('works with one coin', () => {
    expect(printableCoins([coinInChain], testChainInfo)).toEqual(
      `0.001${thinSpace}JUNOX`,
    );
  });
