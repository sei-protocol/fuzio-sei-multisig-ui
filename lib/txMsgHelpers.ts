...
const isTxMsgExecuteContract = (
  msg: TxMsg | EncodeObject,
): msg is TxMsgExecuteContract =>
  msg.typeUrl === '/cosmwasm.wasm.v1.MsgExecuteContract';

const isTxMsgInstantiateContract = (
  msg: TxMsg | EncodeObject,
): msg is TxMsgInstantiateContract =>
  msg.typeUrl === '/cosmwasm.wasm.v1.MsgInstantiateContract';

const isTxMsgMigrateContract = (
  msg: TxMsg | EncodeObject,
): msg is TxMsgMigrateContract =>
  msg.typeUrl === '/cosmwasm.wasm.v1.MsgMigrateContract';

const isTxMsgUpdateAdmin = (
  msg: TxMsg | EncodeObject,
): msg is TxMsgUpdateAdmin =>
...
const isTxMsgClaimRewards = (
  msg: TxMsg | EncodeObject,
): msg is TxMsgClaimRewards =>
...
const isTxMsgSetWithdrawAddress = (
  msg: TxMsg | EncodeObject,
): msg is TxMsgSetWithdrawAddress =>
...
const gasOfTx = (msgTypes: readonly MsgType[]): number => {
  ...
  const totalTxGas = msgTypes.reduce(
    (acc, msgType) => acc + gasOfMsg(msgType),
    txFlatGas,
  );
  return totalTxGas;
};
